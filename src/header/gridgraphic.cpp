...
void gridgraphic::drawGrid(string path, string type, float& opacity, bool& topMost, bool& topBar, bool& autoClose, string& stats, string& interval, bool& fixedStats, bool& selfStats, string& userName, string& sortby) {
	...
	while (!WindowShouldClose()) {
		...
		float scaleX = (float)screenWidth / 800.0f;
		...
		if (shouldEdit) {
			...
			if (!isMinimized) {
				...
				if (type == "bw") {
					...
					DrawTextEx(arialFont, "FKDR", { x + (400 * scaleX), y }, fontSize * scaleX, 2, WHITE);
					DrawTextEx(arialFont, "Ws (All)", { x + (470 * scaleX), y }, fontSize * scaleX, 2, WHITE);
					DrawTextEx(arialFont, "Guild", { x + (550 * scaleX), y }, fontSize * scaleX, 2, WHITE);
					y += yFac;
					...
					for (auto player : players) {
						...
						if (player.serverRank == "Champion")
							tempColor = RED;
						else if (player.serverRank == "Titan")
							tempColor = GOLD;
						else if (player.serverRank == "VIP")
							tempColor = GREEN;
						else if (player.serverRank == "Elite")
							tempColor = { 0, 255, 255, 255 };
						else
							tempColor = GRAY;
						...
						if (player.isNicked)
							...
							if (hasBorder) {
								...
								DrawTextEx(arialRoundFont, fkdrString.c_str(), { x - 1 + (350 * scaleX), y - 1 }, fontSize* scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, fkdrString.c_str(), { x - 1 + (350 * scaleX), y + 1 }, fontSize* scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, fkdrString.c_str(), { x + 1 + (350 * scaleX), y - 1 }, fontSize* scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, fkdrString.c_str(), { x + 1 + (350 * scaleX), y + 1 }, fontSize* scaleX, 2, borderColor);
							...
							DrawTextEx(arialRoundFont, fkdrString.c_str(), { x + (350 * scaleX), y }, fontSize* scaleX, 2, tempColor);
						}
						...
						if (player.isNicked)
							DrawTextEx(arialRoundFont, string("   -").c_str(), { x + (400 * scaleX), y }, fontSize * scaleX, 2, MAROON);
						else {

							if (hasBorder) {

								DrawTextEx(arialRoundFont, wlrString.c_str(), { x - 1 + (400 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, wlrString.c_str(), { x - 1 + (400 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, wlrString.c_str(), { x + 1 + (400 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, wlrString.c_str(), { x + 1 + (400 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
							...
							DrawTextEx(arialRoundFont, wlrString.c_str(), { x + (400 * scaleX), y }, fontSize * scaleX, 2, tempColor);
						}
						...
						if (player.isNicked)
							DrawTextEx(arialRoundFont, string("   -").c_str(), { x + (470 * scaleX), y }, fontSize * scaleX, 2, MAROON);
						else {

							if (hasBorder) {

								DrawTextEx(arialRoundFont, to_string(player.wsLifetime).c_str(), { x - 1 + (470 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, to_string(player.wsLifetime).c_str(), { x - 1 + (470 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, to_string(player.wsLifetime).c_str(), { x + 1 + (470 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, to_string(player.wsLifetime).c_str(), { x + 1 + (470 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
							...
							DrawTextEx(arialRoundFont, to_string(player.wsLifetime).c_str(), { x + (470 * scaleX), y }, fontSize * scaleX, 2, tempColor);
						}

						if (player.isNicked)
							DrawTextEx(arialRoundFont, string("-").c_str(), { x + (550 * scaleX), y }, fontSize * scaleX, 2, MAROON);
						else {

							if (hasBorder) {

								DrawTextEx(arialRoundFont, player.guildTag.c_str(), { x - 1 + (550 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, player.guildTag.c_str(), { x - 1 + (550 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, player.guildTag.c_str(), { x + 1 + (550 * scaleX), y - 1 }, fontSize * scaleX, 2, borderColor);
								DrawTextEx(arialRoundFont, player.guildTag.c_str(), { x + 1 + (550 * scaleX), y + 1 }, fontSize * scaleX, 2, borderColor);
							...
							DrawTextEx(arialRoundFont, player.guildTag.c_str(), { x + (550 * scaleX), y }, fontSize * scaleX, 2, { 0, 255, 255, 255 });
						}
